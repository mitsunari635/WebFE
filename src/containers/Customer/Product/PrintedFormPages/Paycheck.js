...
class Paycheck extends Component {
  ...
  componentDidMount() {
    this.props.loadPaycheck();
  }

  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevProps.PaychecksRedux !== this.props.PaychecksRedux) {
      this.setState({
        arrPaychecks: this.props.PaychecksRedux,
      });
    }
  }
  ...
  render() {
    let arrPaychecks = this.state.arrPaychecks;
    return (
      <>
        <HomeHeader />
        <Tab />
        <div className="product-page">
          <div className="product-content">
            <div className="product-header">
              <span className="title-section">Phiếu lương</span>
            </div>
            ...
            <div className="product-body">
              {arrPaychecks &&
                arrPaychecks.length > 0 &&
                arrPaychecks.map((item, index) => {
                  let imageBase64 = "";
                  if (item.image) {
...
const mapStateToProps = (state) => {
  return {
    PaychecksRedux: state.product.paychecks,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    loadPaycheck: () => dispatch(actions.fetchPaycheck()),
  };
};

export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Paycheck)
);
