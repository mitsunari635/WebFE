...
class EdcBill extends Component {
  constructor(props) {
    super(props);
    this.state = {
      arrEdcBills: [],
  ...
  componentDidMount() {
    this.props.loadEdcBill();
  }

  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevProps.EdcBillsRedux !== this.props.EdcBillsRedux) {
      this.setState({
        arrEdcBills: this.props.EdcBillsRedux,
      });
    }
  }
  ...
  render() {
    let arrEdcBills = this.state.arrEdcBills;
    return (
      <>
        <HomeHeader />
        <Tab />
        <div className="product-page">
          <div className="product-content">
            <div className="product-header">
              <span className="title-section">Hóa đơn EDC / cà thẻ</span>
            </div>
            ...
            <div className="product-body">
              {arrEdcBills &&
                arrEdcBills.length > 0 &&
                arrEdcBills.map((item, index) => {
                  let imageBase64 = "";
                  if (item.image) {
...
const mapStateToProps = (state) => {
  return {
    EdcBillsRedux: state.product.edcBills,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    loadEdcBill: () => dispatch(actions.fetchEdcBill()),
  };
};

export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(EdcBill)
);
